  ...
  return (
    <div className="bg-slate-800 p-4 rounded-lg shadow-md space-y-3">
      <div className="flex justify-between items-start">
        <div>
          <h3 className="text-xl font-bold text-indigo-400">{channel.name}</h3>
          <a href={channel.url} target="_blank" rel="noopener noreferrer" className="text-sm text-slate-400 hover:text-indigo-400 break-all">{channel.url}</a>
        </div>
...
const ChannelManager: React.FC<ChannelManagerProps> = ({ channels, setChannels }) => {
  ...
  const handleSave = (channelData: Omit<Channel, 'id'>) => {
    if (editingChannel) {
      setChannels(channels.map(c => c.id === editingChannel.id ? { ...c, ...channelData } : c));
    } else {
      const newChannel: Channel = { id: Date.now().toString(), ...channelData };
    ...
    <div>
      ...
      <div className="space-y-4">
        {channels.length > 0 ? (
          ...
          <p className="text-slate-400 text-center py-10 text-lg">No channels yet. Click 'Add Channel' to get started.</p>
        )}
      </div>
